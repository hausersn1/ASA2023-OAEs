function [res] = SFanalysis_newNR(subj, ear, windowdur, offsetwin, npoints)
...
for x = 1:stim.Averages
    ...
    oae_a(x, :) = a; 
    oae_b(x, :) = b; 
    oae(x, :) = abs(complex(a, b) .* phasor); %  .* stim.VoltageToPascal .* stim.PascalToLinearSPL;
    ...
    %fx = (f_tau(2:end) + f_tau(1:end-1))/2;
end

An = sqrt(mean(oae_a, 1).^2 + mean(oae_b, 1).^2); 
se_a = std(oae_a, [], 1)/sqrt(size(oae_a, 1)); 
se_b = std(oae_b, [], 1)/sqrt(size(oae_b, 1)); 

se_An = sqrt(se_a.^2 + se_b.^2)/sqrt(2); 
